"""
Soft preference distillation for Flow-Planner.
==============================================

This stage sits between goal-conditioned base training and hard-pair DPO:

1. Generate K goal-conditioned candidates per scene.
2. Score every candidate with the structured scorer.
3. Convert per-scene candidate scores into a soft target distribution.
4. Fine-tune the decoder LoRA weights to match that soft distribution.

Compared with hard DPO, this stage uses every candidate in the scene instead
of only the final chosen/rejected pair.
"""
...
class SoftPreferenceSceneDataset(Dataset):
    """Load per-scene scored candidates and build soft target distributions."""
    ...
    def __init__(
        ...
        self.require_goals = require_goals
        ...
        for scored_path in scored_files:
            ...
            goal_labels = None
            if "goal_labels" in raw.files:
                goal_labels = np.asarray(raw["goal_labels"], dtype=np.float32)
            if require_goals and goal_labels is None:
                skipped["missing_goals"] += 1
                continue
        ...
        if not self.records:
            raise RuntimeError(
                "No valid scored scenes were loaded for soft preference distillation. "
                "Check scored_dir, scene_dir, and candidate paths."
            )

        logger.info(
            "Loaded %d scored scenes for soft preference distillation (K=%d).",
            len(self.records),
    ...
    def __getitem__(self, idx: int) -> Dict[str, object]:
        ...
        if goals is not None:
            goals_tensor = torch.from_numpy(np.asarray(goals, dtype=np.float32)).float()

        return {
            "scenario_id": record["scenario_id"],
            "trajectories": torch.from_numpy(
                np.asarray(record["trajectories"], dtype=np.float32)
            ).float(),
            "target_probs": torch.from_numpy(
                np.asarray(record["target_probs"], dtype=np.float32)
            ).float(),
            "top_idx": int(record["top_idx"]),
            "gt_similarity": torch.from_numpy(
                np.asarray(record["gt_similarity"], dtype=np.float32)
            ).float(),
            "score_values": torch.from_numpy(
                np.asarray(record["score_values"], dtype=np.float32)
            ).float(),
...
def collate_soft_preferences(batch: List[Dict[str, object]]) -> Dict[str, object]:
    trajectories = torch.stack([item["trajectories"] for item in batch])  # (B, K, T, D)
    target_probs = torch.stack([item["target_probs"] for item in batch])  # (B, K)
    gt_similarity = torch.stack([item["gt_similarity"] for item in batch])  # (B, K)
    score_values = torch.stack([item["score_values"] for item in batch])  # (B, K)
    raw_scores = torch.stack([item["raw_scores"] for item in batch])  # (B, K)
    top_idx = torch.tensor([item["top_idx"] for item in batch], dtype=torch.long)
    scenario_ids = [str(item["scenario_id"]) for item in batch]

    goals = None
    if all(item.get("goals") is not None for item in batch):
        goals = torch.stack([item["goals"] for item in batch])  # (B, K, 2)
...
def compute_scene_candidate_log_probs(
    ...
    """Evaluate all K candidates in a scene batch."""
    batch_size, num_candidates = trajectories.shape[:2]
    log_probs: List[torch.Tensor] = []
    for idx in range(num_candidates):
        candidate = trajectories[:, idx].to(device)
        goal_point = goals[:, idx] if goals is not None else None
        conditioned = attach_goal_to_decoder_inputs(encoder_outputs, goal_point, device)
...
def train_soft_preferences(args: argparse.Namespace) -> None:
    logging.basicConfig(
        level=logging.INFO,
    ...
    logger.info("=" * 60)
    logger.info("Loading scored scenes...")
    dataset = SoftPreferenceSceneDataset(
        ...
        require_goals=args.require_goals,
    )
    ...
    dataloader = DataLoader(
        ...
        pin_memory=True,
        drop_last=True,
        collate_fn=collate_soft_preferences,
    )
    ...
    logger.info("=" * 60)
    logger.info("Starting soft preference distillation:")
    ...
    for epoch in range(args.epochs):
        ...
        for batch in pbar:
            ...
            log_probs = compute_scene_candidate_log_probs(
                model=policy_model,
                trajectories=trajectories,
            ...
            if ref_model is not None and args.ref_kl_weight > 0.0:
                with torch.no_grad():
                    ref_log_probs = compute_scene_candidate_log_probs(
                        model=ref_model,
                        trajectories=trajectories,
                        encoder_outputs=encoder_outputs,
                        goals=goals,
    ...
    logger.info("=" * 60)
    logger.info("Soft preference distillation complete!")
    ...
    if args.save_merged:
        ...
        merged_path = os.path.join(args.output_dir, "model_softpref_merged.pth")
        raw_state_dict = policy_model.state_dict()
        fixed_state_dict = {k.replace("module.", ""): v for k, v in raw_state_dict.items()}
        torch.save({"state_dict": fixed_state_dict}, merged_path)
        logger.info("Merged model saved to %s (%d keys)", merged_path, len(fixed_state_dict))
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Soft preference distillation for Flow-Planner",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )

    parser.add_argument("--config_path", type=str, required=True, help="Model config YAML path")
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Per-scene JSONs emitted by score_hybrid.py --emit_traj_info",
    )
    parser.add_argument(
        "--scene_dir",
        type=str,
        required=True,
        help="Directory of raw scene NPZs used to rebuild encoder condition",
    )
    parser.add_argument(
        "--candidates_dir",
        type=str,
        default=None,
        help="Fallback directory for *_candidates.npz when source_npz paths are not valid",
    )
    parser.add_argument("--max_scenes", type=int, default=None, help="Max scored scenes to use")
    parser.add_argument("--min_candidates", type=int, default=2, help="Min candidates per scene")
    parser.add_argument(
        "--require_goals",
        action="store_true",
        help="Skip scenes whose candidate NPZ lacks goal_labels",
    )

    parser.add_argument("--output_dir", type=str, default="checkpoints/soft_pref")
    ...
    parser.add_argument(
        ...
        type=int,
        default=8,
        help="Number of flow-matching timesteps used for log-prob estimation",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Temperature for the soft target distribution",
    )
    parser.add_argument(
        "--gt_weight",
        type=float,
        default=0.5,
        help="Weight of GT similarity term in the soft target",
    )
    parser.add_argument(
        "--score_weight",
        type=float,
        default=1.0,
        help="Weight of scorer total_score term in the soft target",
    )
    parser.add_argument(
        "--gt_ade_weight",
        type=float,
        default=0.5,
        help="Weight of ADE in the GT similarity proxy",
    )
    parser.add_argument(
        "--gt_fde_weight",
        type=float,
        default=1.0,
        help="Weight of FDE in the GT similarity proxy",
    )
    parser.add_argument(
        "--top1_weight",
        type=float,
        default=0.1,
        help="Extra weight for the teacher top-1 candidate log-prob anchor term",
    )
    parser.add_argument(
        "--ref_kl_weight",
        type=float,
        default=0.0,
        help="Optional KL regularization toward the frozen base policy distribution",
    )
